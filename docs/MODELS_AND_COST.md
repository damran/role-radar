...
| `CV_MODEL` | Phase 3 | `P3: OpenRouter Generate CV` | `anthropic/claude-sonnet-4.6` | Highest-quality tailored CV. |
| `CL_MODEL` | Phase 3 | `P3: OpenRouter Generate Cover Letter` | `anthropic/claude-sonnet-4.6` | Human-sounding cover letter. |
...
>
> 🚫 **Use a concrete versioned slug, not a `…-latest` alias.** OpenRouter lists floating aliases like
> `anthropic/claude-sonnet-latest` with a `~` prefix and they are **not directly callable** — sending one
> returns `400 … is not a valid model ID`. Pin the version (e.g. `anthropic/claude-sonnet-4.6`).
